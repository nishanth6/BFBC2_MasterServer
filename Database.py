...
from Framework.ErrorCodes import ERROR_FAILED_TO_INITIALIZE_DATABASE
...
class Database(object):
    def __init__(self, showWelcomeMsg=False):
        dbFileLocation = readFromConfig("database", "file_path")

        if exists(dbFileLocation):
            if showWelcomeMsg:
                logger.notification('Connected to database!', 2)
            self.connection = sqlite3.connect(dbFileLocation)
            self.cleanup()
        else:
            logger.notification('Database file not found! Initializing database...', 1)
            self.connection = sqlite3.connect(dbFileLocation)
            try:
                self.initializeDatabase()
                logger.notification('Database initialized successfully!', 1)
            except Exception as DBError:
                logger.error('There is an problem with initializing database!\nAdditional Error Info:\n' + str(DBError),
                             1)
                sys.exit(ERROR_FAILED_TO_INITIALIZE_DATABASE)
    ...
    def initializeDatabase(self):
        tables = [{'Accounts': ['userID integer PRIMARY KEY AUTOINCREMENT UNIQUE', 'EMail string UNIQUE',
                  ...
                  {'BlockedPlayers': ['personaID integer', 'concernPersonaID integer', 'type integer',
                                      'creationDate string']},
                  {'Entitlements': ['userID integer', 'groupName string',
                                    'entitlementId integer PRIMARY KEY AUTOINCREMENT UNIQUE', 'entitlementTag string',
                                    'version integer', 'grantDate string', 'terminationDate string', 'productId string',
                                    'status string', 'statusReasonCode string']},
                  {'MutedPlayers': ['personaID integer', 'concernPersonaID integer', 'type integer',
                                    'creationDate string']},
                  {'RecentPlayers': ['personaID integer', 'concernPersonaID integer', 'type integer',
                                     'creationDate string']},
                  {'Stats': ['personaID integer', 'key string', 'value integer']},
                  {'UsersFriends': ['personaID integer', 'concernPersonaID integer', 'type integer',
                                    'creationDate string']},
                  {'UsersMessages': ['senderID integer', 'receiverID integer', 'messageType string',
                                     'messageID integer PRIMARY KEY AUTOINCREMENT UNIQUE', 'attachments string',
                                     'timeSent string', 'expiration integer', 'deliveryType string',
                                     'purgeStrategy string']},
                  {'Personas': ['personaID integer PRIMARY KEY AUTOINCREMENT UNIQUE', 'userID integer',
                                'personaName string']}]
        ...
        cursor.execute("INSERT INTO sqlite_sequence (name, seq) VALUES (?,?)",
                       ("Accounts", 3,))  # Start counting Accounts from 4 (3 are already occupied by server(s))
        cursor.execute("INSERT INTO sqlite_sequence (name, seq) VALUES (?,?)",
                       ("Personas", 3,))  # Start counting Personas from 4 (3 are already occupied by server(s))
        self.connection.commit()
        cursor.close()
    ...
    def registerUser(self, email, password, birthday, country):
        ...
        try:
            ...
            logger.notification('Successfully registered new account (' + email + ')!', 1)
            return True
        except:
            logger.warning('User with this email (' + email + ') are currently registered!', 1)
            return False
    ...
    def addDefaultEntitlements(self, forUserID):
        ...
        defaultEntitlements = [(forUserID, 'NoVetRank', "BFBC2NAM:PC:NOVETRANK", 0, currentTime, "", "", "ACTIVE", "",),
                               ...
                               (
                               forUserID, 'AddsVetRank', "BFBC2:PC:ADDSVETRANK", 0, currentTime, "", "", "ACTIVE", "",),
                               # sometimes clients don't have this in the packet...whatever
                               (forUserID, 'BFBC2PC', 'BETA_ONLINE_ACCESS', 0, currentTime, "", "OFB-BFBC:19121",
                                "ACTIVE", "",)]  # beta access is nice too (though it doesnt seem to affect anything)

        if readFromConfig("players", "new_players_have_vietnam"):
            defaultEntitlements.append(
                (forUserID, 'BFBC2PC', "BFBC2:PC:VIETNAM_ACCESS", 0, currentTime, "", "DR:219316800", "ACTIVE", ""))
            defaultEntitlements.append(
                (forUserID, 'BFBC2PC', "BFBC2:PC:VIETNAM_PDLC", 0, currentTime, "", "DR:219316800", "ACTIVE", ""))

        if readFromConfig("players", "new_players_have_premium"):
            defaultEntitlements.append(
                (forUserID, 'BFBC2PC', "BFBC2:PC:LimitedEdition", 1, currentTime, "", "OFB-BFBC:19120", "ACTIVE", ""))

        if readFromConfig("players", "new_players_have_specact"):
            defaultEntitlements.append(
                (forUserID, 'BFBC2PC', "BFBC2:PC:ALLKIT", 0, currentTime, "", "DR:192365600", "ACTIVE", ""))

        if readFromConfig("players", "new_players_are_veterans"):
            defaultEntitlements.append(
                (forUserID, 'AddsVetRank', "BF3:PC:ADDSVETRANK", 0, currentTime, "", "OFB-EAST:40873", "ACTIVE", ""))
    ...
    def AddAssociations(self, toPersonaID, ownerID, ownerType, type):
        ...
        cursor = self.connection.cursor()
        cursor.execute("INSERT INTO " + type + " (personaID, concernPersonaID, type, creationDate) VALUES (?,?,?,?)",
                       (ownerID, toPersonaID, ownerType, currentTime))
    ...
    def sendMessage(self, fromPersona, toUsers, messageType, attachments, expires, deliveryType, purgeStrategy):
        ...
        for user in toUsers:
            cursor = self.connection.cursor()
            cursor.execute(
                "INSERT INTO UsersMessages (senderID, receiverID, messageType, attachments, timeSent, expiration, deliveryType, purgeStrategy) VALUES (?,?,?,?,?,?,?,?)",
                (fromPersona, user, str(messageType), str(attachments), str(currentTime), expires, str(deliveryType),
                 str(purgeStrategy),))
            ...
            cursor = self.connection.cursor()
            cursor.execute("SELECT messageID FROM UsersMessages WHERE senderID = ? AND receiverID = ? AND timeSent = ?",
                           (fromPersona, toUsers[0], currentTime,))
            data = cursor.fetchone()
        ...
        if data is not None:
            for message in data:
                ...
                if personaName is not False:
                    ...
                    logger.warning("WARNING: Found incorrect message! Removing it...", 2)
